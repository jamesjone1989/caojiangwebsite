...
test("server-renders the Cao Jiang works map", async () => {
  ...
  assert.match(html, /你好<br\/>我是<br\/><span>曹将<\/span>/);
  assert.match(
    html,
  ...
  assert.doesNotMatch(html, /继续表达。/);
  assert.doesNotMatch(html, /这里是曹将。也欢迎你从任何一个入口，继续认识我。/);
...
test("keeps the finished visual system and removes starter assets", async () => {
  ...
  assert.ok(cardHeights.every(height => Number(height) >= 430), "preserve the tall portrait card treatment");
