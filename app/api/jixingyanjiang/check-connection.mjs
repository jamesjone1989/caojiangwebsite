...
import { sendCompletion, upstreamError, connectionFailure } from './provider-request.mjs';
...
export async function checkConnection(input = {}, fetcher = fetch) {
  ...
  try {
    const response = await sendCompletion(config,[{role:'user',content:'Reply OK.'}],{maxTokens:config.resolvedProvider === 'openai' ? 1024 : 16,json:false,timeout:35_000},fetcher);
